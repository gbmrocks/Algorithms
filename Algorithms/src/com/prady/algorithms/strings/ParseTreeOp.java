...
/*public static String simplify(String input){       
    ...
    /*for(int i=0; i<inputArr.length; i++){
        if(inputArr[i]=='(' && (i==0 || inputArr[i]==inputArr[i-1])){
            inputArr[i] = ' ';
            flag = true;
        }else if(flag && inputArr[i]==')'){
            inputArr[i] = ' ';
    ...
    /*for(int i=0; i<inputArr.length; i++){
        ...
        if(currentChar == '('){
            if(i==0 || flag){
                bracketStk.push(currentChar);
                flag = true;
            }else if(!inputStack.isEmpty() && inputStack.peek() == '('){
                bracketStk.push(currentChar);
            }else{
                inputStack.push(currentChar);
            }
        }else if(currentChar == ')'){
            if(flag && !bracketStk.isEmpty()){
                bracketStk.pop();
                if(bracketStk.isEmpty())
                    flag = false;
            }else if(!bracketStk.isEmpty() && bracketStk.peek()=='('){
                bracketStk.pop();   
            }else{
                inputStack.push(currentChar);
            }
    ...
    /*for(int i=0; i<inputArr.length; i++){
        ...
        if(i==0 && inputArr[i]=='('){
            flag = true;
        }else if(inputArr[i]=='(' && !inputStack.isEmpty() && inputStack.peek() == '('){
            flag = true;
        }else if(inputArr[i]==')' && flag){
            flag=false;
        }else{
            inputStack.push(inputArr[i]);
        }
    }*/
    
    /*result = new char[inputStack.size()];
    int i = inputStack.size()-1;
    while(!inputStack.isEmpty() && i>=0){
        result[i] = inputStack.pop();
        i--;
    }*/
    /*if(inputArr.length > result.length)
        result = simplify(new String(result)).toCharArray();*/
    /*return new String(inputArr).replaceAll(" ","");
}*/
...
/*public static String simplify2(String input){
    ...
    for(int i=0; i<inputArr.length; i++){
        if(i != inputArr.length-1 && inputArr[i] == '(' && inputArr[i+1] != '('){
            stack.push(inputArr[i]);
        }else if(inputArr[i] == ')' && !stack.isEmpty()){
            stack.pop();
        }else{
            buf.append(inputArr[i]);
        }
    }
    return buf.toString();
}*/
